...
def get_model(config):
    ...
    """
    model = build_kanformer(config.src_voc_size, config.tgt_voc_size, (max(config.src_max_len,config.tgt_max_len) + 128), 
                            config.device, config.embedding_size, config.num_layers, 
                            config.nhead,  config.ff_dims)
